...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _Inputview=[[UIPlaceholderTextView alloc]initWithFrame:textFrame PlaceText:@"评论" PlaceColor:[UIColor lightGrayColor]];
        _Inputview.selfDelegate=self;
...
#pragma mark 代理处理来自输入框的大小
-(void)TextViewDidChange:(CGSize)size{
    self.frame=CGRectMake(0, self.frame.origin.y, self.frame.size.width,size.height+6*2);
    self.frame=CGRectMake(0,-self.frame.size.height+maxy, self.frame.size.width, self.frame.size.height);
